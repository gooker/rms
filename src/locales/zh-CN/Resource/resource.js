export default {
  'resourceBind.horizontalaxis': '横轴',
  'resourceBind.verticalaxis': '纵轴',
  'resourceBind.repeat.vertivalaxis': '不能和纵轴重复',
  'resourceBind.repeat.horizeontalaxis': '不能和横轴重复',

  // 分组操作
  'resourceGroup.grouping': '分组',
  'resourceGroup.operate': '组操作',
  'resourceGroup.add': '添加到组',
  'resourceGroup.delete': '删除分组',
  'resourceGroup.editPriority': '编辑优先级',
  'resourceGroup.deleteMember': '从组中删除',
  'resourceGroup.deleteMember.confirm': '是否确定从组中删除?',
  'resourceGroup.nogrouping': '不存在分组',

  // 载具
  'load.specification': '载具规格',
  'load.group': '载具组',

  // 储位列表
  'storage.template': '储位模版',
  'storage.group': '储位组',
};
