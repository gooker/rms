export default {
  'latentTote.podTemplateStorage': '货架储位',
  'latentTote.podTemplateStorage.weight': '货架重量',
  'latentTote.podTemplateStorage.rowSpace': '层板厚度',
  'latentTote.podTemplateStorage.storageWeight': '储位承重',
  'latentTote.podTemplateStorage.bearingTotalweight': '承载总重',
  'latentTote.podTemplateStorage.required': '输入',
  'latentTote.podTemplateStorage.bearWeight.required': '请检查储位承重',
  'latentTote.podTemplateStorage.column': '列',
  'latentTote.podTemplateStorage.storage.tip': '必须选择连续层',

  // 任务管理
  'latentTote.mainStationCode': '工作站组',
  'latentTote.targetTime': '目标时间',

  // 订单状态
  'latentTote.orderStatus.SENT': '已发送',
  'latentTote.orderStatus.GRABBING': '机械臂抓取中',
  'latentTote.orderStatus.GRABBING_FINISH': '机械臂抓取完成',
  'latentTote.orderStatus.TALLY_TRANSPORTING': '理货运输中',
  'latentTote.orderStatus.TALLYING': '理货中',
  'latentTote.orderStatus.TALLIED': '理货完成',
  'latentTote.orderStatus.PICK_STATION_TRANSPORTING': '出库运输中',
  'latentTote.orderStatus.PICK_STATION_WORKING': '出库中',

  // 工作站
  'latentTote.station.type.PICK': '拣货工作站',
  'latentTote.station.type.PUT_AWAY': '上架工作站',

  'latentTote.station.maxPod': '最大接pod数',
  'latentTote.station.simulatedStatus': '是否模拟工作站',
  'latentTote.station.workStatus': '工作状态',
  'latentTote.station.workModel': '工作模式',

  'latentTote.station.workModel.PICK': '拣货',
  'latentTote.station.workModel.TALLY': '理货',
  'latentTote.station.workModel.STORE_PREPARE': '上架-开班呼叫',
  'latentTote.station.workModel.STORE': '上架',

  'latentTote.station.errorLevel': '异常等级',
  'latentTote.station.errorCode': '异常编码',
  'latentTote.station.errorInfo': '异常记录',
  'latentTote.station.errorContent': '异常内容',
};
