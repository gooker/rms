...
export const taskLineOption = (title, keyMap) => ({
  ...
  yAxis: {
    type: 'value',
    name:
      title === formatMessage({ id: 'reportCenter.vehicleload.taskDistance' })
...
export const generateDurationDataByTime = (
  ...
  Object.entries(currentSery).forEach((key, i) => {
    series.push({
      ...
      type: 'bar',
      itemStyle: {
        color: colorFlag ? VehicleStateColor[key[0]] ?? null : null, //null会是取默认颜色，key[1]?.[0]?.color,
      },
    });
  });
...
// table数据--根据vehicleId
export const generateTableData = (originalData = {}, vehicleData = []) => {
  const currentAxisData = [...vehicleData];
  ...
  let currentCellIdData = {}; // 根据vehicleId 每个key 求和
  ...
  const currentVehicleData = Object.values(currentCellIdData);
