...
class VehicleActivityForm extends Component {
  ...
  render() {
    const { defaultValue, mode, disabled, vehicleType, allTaskTypes } = this.props;
    return (
      <Form
        layout="inline"
                ...
                <Row>
                  <Col span={12}>
                    <Form.Item
                      ...
                      initialValue={defaultValue ? defaultValue.vehicleTaskType : []}
                    >
                      <Select
                        disabled={disabled}
                        allowClear
                        ...
                        maxTagCount={1}
                        options={(() => {
                          const specificVehicleTaskTypes = allTaskTypes?.[vehicleType] || {};
                          return Object.keys(specificVehicleTaskTypes).map((type) => ({
                            label: specificVehicleTaskTypes[type],
                            value: type,
                            key: type,
                          }));
                        })()}
