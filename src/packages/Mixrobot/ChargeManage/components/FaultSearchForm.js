      ...
      Object.keys(values).forEach((formKey) => {
        if (formKey === 'date') {
          if (!isNull(values?.date?.[0])) {
            formValues.createTimeStart = GMT2UserTimeZone(values.date[0]).format('YYYY-MM-DD HH:mm:ss');
          }
          if (!isNull(values?.date?.[1])) {
            formValues.createTimeEnd = GMT2UserTimeZone(values.date[1]).format('YYYY-MM-DD HH:mm:ss');
          }
        } else {
