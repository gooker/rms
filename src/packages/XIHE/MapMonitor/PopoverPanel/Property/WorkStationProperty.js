...
import { isStrictNull, StationStateColor, formatMessage, isNull, dealResponse } from '@/utils/util';
            ...
            <div>
              {!isStrictNull(data.status) && (
                <Tag color={StationStateColor[data.status]}>
                  <FormattedMessage id={`app.agvState.${data.status}`} />
                </Tag>
              )}
            </div>
          </div>
